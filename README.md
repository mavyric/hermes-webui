...
## Fork feature log (mavyric)

This fork (`mavyric/hermes-webui`) tracks upstream
[nesquena/hermes-webui](https://github.com/nesquena/hermes-webui) —
[upstream README](https://github.com/nesquena/hermes-webui#readme) remains the
baseline documentation — and is re-synced to it daily by the
`sync-with-upstream` GitHub Actions workflow. This section records the
features this fork adds on top of upstream.
...
- **CoT dedup at rest** — reasoning is persisted once (`reasoning` is
  ...
  Measured on the real ~1.9 GB R9V sidecar: **1.899 GB → 0.737 GB** with all
  261,158 messages and 260,582 CoT blocks preserved.
...
- The UI renders a collapsed Thinking card and fetches only the records it
  ...
  (R9V: full-load peak ~7.4 GiB → ~0.3 GiB; sidecar → ~72 MB).
...
- [Fork feature log](#fork-feature-log-mavyric) — features this fork adds over upstream
